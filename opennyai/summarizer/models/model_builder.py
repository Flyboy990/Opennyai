    ...
    def __init__(self, large, temp_dir=EXTRACTIVE_SUMMARIZER_CACHE_PATH, finetune=False):
      ...
      def forward(self, x, segs, mask):
          if (self.finetune):
           ...
           # Extract last_hidden_state from outputs (works with both old and new transformers)
...
class ExtSummarizer(nn.Module):
    def __init__(self, args, device, checkpoint):
        ...
        if self.args.use_rhetorical_roles:
            self.ext_layer = ExtTransformerEncoder(self.bert.model.config.hidden_size + 16, args.ext_ff_size,
                                                   args.ext_heads,
